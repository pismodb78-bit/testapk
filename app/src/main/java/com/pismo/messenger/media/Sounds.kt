 ...
 *
 * ОТЛИЧИЕ ОТ ПК: маршрут вывода выбирается по обстановке. В разговоре звук
 * идёт по голосовому тракту (USAGE_VOICE_COMMUNICATION) — иначе на телефоне
 * он ушёл бы в динамик мимо гарнитуры и в наушниках его бы просто не было.
 * Вне разговора — обычный системный тракт.
 */
object Sounds {
    ...
    fun callConnected() = play("conn") { doubleGlide(300.0, 520.0, 420.0, 660.0, 150, 0.2) }
    ...
    private suspend fun blast(pcm: ShortArray) {
        // В разговоре — по голосовому тракту, иначе мимо гарнитуры.
        val inCall = com.pismo.messenger.call.ActiveCall.engine != null
        val attrs = AudioAttributes.Builder()
            .setUsage(
                if (inCall) AudioAttributes.USAGE_VOICE_COMMUNICATION
