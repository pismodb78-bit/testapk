...
object Prefs {
    ...
    /**
     * Push уже забрал работу у фоновой проверки — однократно.
     *
     * Фоновая проверка держит службу, служба обязана держать уведомление, и
     * убрать его приложение не может: так устроен Android. Push снимает саму
     * причину — сообщения, звонки и каналы приносит система, служба не нужна,
     * шторка чистая.
     *
     * Пометка нужна, чтобы выключить проверку РОВНО ОДИН раз, когда push
     * впервые заработал. Без неё выключатель в настройках терял бы смысл:
     ...
     *
     * Хранятся В БАЗЕ (таблица chat_pins) и потому одни и те же на телефоне
     * и на компьютере: раньше каждый клиент держал свой список у себя, и один
     * и тот же человек видел разный порядок на разных устройствах.
     *
     * Здесь остался КЕШ: порядок строк спрашивают при каждой отрисовке, а
     * база отвечает не мгновенно и не всегда. Ключ включает id аккаунта,
    ...
    /** Перезаписать кеш закрепов целиком — тем, что приехало из базы. */
    fun setPinnedChats(ids: Set<Int>) {
        sp.edit().putString(pinnedKey(), ids.joinToString(",")).apply()
    }

    /**
     * Старые закрепы этого телефона уже перенесены в общее хранилище?
     *
     * Метка обязательна: без неё то, что человек открепил на компьютере,
     * воскресало бы при каждом запуске телефона — местный список каждый раз
     * доливался бы в базу заново.
     */
    private fun pinsSyncedKey() = "pinned_chats_synced_${UserSession.effectiveId}"

    var pinsSynced: Boolean
        get() = sp.getBoolean(pinsSyncedKey(), false)
        set(v) = sp.edit().putBoolean(pinsSyncedKey(), v).apply()

    /** Закрепить/открепить чат в КЕШЕ. Возвращает НОВОЕ состояние. */
    ...
    /**
     * Чем жертвовать, когда канала не хватает: кадрами или чёткостью.
     *
     * true (по умолчанию) — держим частоту кадров, WebRTC сам понижает
     * разрешение. Это MAINTAIN_FRAMERATE, и именно он отвечает за плавность:
     * с обратной настройкой картинка остаётся резкой, но при нехватке канала
     * начинает идти рывками — SDK для демонстрации по умолчанию выбирает
     * как раз её.
     *
     * false — MAINTAIN_RESOLUTION: чёткий текст ценой рывков. Разумно, когда
     * показывают код или документ, где важна каждая буква.
     */
    var screenShareSmooth: Boolean
        get() = sp.getBoolean("screen_share_smooth", true)
        set(v) = sp.edit().putBoolean("screen_share_smooth", v).apply()
    ...
    val screenShareBitrate: Int
        get() = when (screenShareQuality) {
            ...
            //
            // Потолки подняты: это именно ПОТОЛОК, а не постоянный расход.
            // WebRTC берёт из него ровно столько, сколько нужно картинке и
            // выдерживает канал, а низкий потолок он не обходит никогда — и
            // на экране телефона в два с половиной мегапикселя прежние
            // четыре мегабита были той самой ватой на мелком тексте.
            2 -> 14_000_000
            1 -> 10_000_000
            else -> 6_000_000
        }
