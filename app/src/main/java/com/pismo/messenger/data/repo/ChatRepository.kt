...
object ChatRepository {
    ...
    suspend fun setChatPin(targetId: Int, pinned: Boolean) {
        ...
        // И сообщаем СВОИМ ЖЕ остальным устройствам — отсюда адресат «me»,
        // а не 0.
        //
        // С нулём это была рассылка ВСЕМ подключённым: каждый получал чужое
        // событие и перезагружал свой список чатов. Закреплённые чаты — вещь
        // личная, посторонним она не нужна вовсе. Релей на адресное сообщение
        // самому себе отдаёт его остальным своим устройствам, минуя
        // отправителя, — ровно то, что нужно.
        runCatching { SignalingClient.send("chatpin", me, targetId, "") }
    }
