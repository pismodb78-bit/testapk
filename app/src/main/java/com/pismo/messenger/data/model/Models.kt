...
data class CallSessionRow(
    ...
    /** Цвет аватара группы — чтобы в окне звонка она выглядела как в списке. */
...
fun Presence.headerText(): String = when {
    ...
    //
    // Поэтому спящий с живым приложением висит жёлтым «бездействует 16 ч», а
    // когда процесс наконец умирает — «был в сети» отсчитывается от heartbeat,
    // а не от последнего нажатия. Это не оплошность, а разделение смыслов.
