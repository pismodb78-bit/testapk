...
class SpectralDenoiser {
     ...
     * вычитания (1.0…2.5) и «полом» подавления (−10…−20 дБ). Ниже пола не
     * опускаемся никогда: полная вырезка полосы и даёт звенящие артефакты.
     */
    @Volatile
    ...
    private fun processHop(inOffset: Int) {
        ...
        val overSub = 1f + 1.5f * s            // 1.0 … 2.5
        val floorGain = 0.32f - 0.22f * s      // −10 … −20 дБ
