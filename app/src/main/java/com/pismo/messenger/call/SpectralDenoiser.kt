...
class SpectralDenoiser(sampleRate: Int = 48_000) {
    ...
    /** Доля отсчётов кадра, упёршихся в потолок разрядной сетки. См. suppressWind. */
    private var clipFrac = 0f
    ...
    private fun suppressWind() {
        ...
        // ОЧЕНЬ СИЛЬНОЕ ЗАДУВАНИЕ — отдельный случай, и раньше оно проходило
        // насквозь именно потому, что было сильным. Струя воздуха такой силы
        // загоняет микрофон в ограничение, а ограничение — это срезанная
        // синусоида, то есть гармоники по всему спектру. Наклон, по которому
        // мы задувание и узнаём, при этом выпрямляется: энергия появляется и
        // наверху, отношение низов к голосовой полосе падает, проверка не
        // срабатывает. Получалось, что чем сильнее дуют, тем меньше шансов,
        // что это заметят.
        //
        // Поэтому при заметном ограничении достаточно куда меньшего перекоса:
        // речь в микрофон не «упирается в потолок» целыми кадрами, а если и
        // упрётся, низов у неё столько не будет.
        if (clipFrac > CLIP_FRAC && tilt > WIND_TILT_CLIPPED) target = 1f
    ...
    private fun processHop(inOffset: Int) {
        ...
        // Сколько свежих отсчётов упёрлось в потолок. Нужно детектору
        // задувания: сильная струя воздуха загоняет микрофон в ограничение.
        var clipped = 0
        for (i in N - HOP until N) {
            val a = if (window[i] < 0f) -window[i] else window[i]
            if (a >= CLIP_LEVEL) clipped++
        }
        clipFrac = clipped.toFloat() / HOP
    ...
    private companion object {
        ...
        /** С какого уровня считаем отсчёт упёршимся в потолок. */
        const val CLIP_LEVEL = 30000f

        /** Какой доли упёршихся отсчётов в кадре хватает, чтобы заподозрить струю. */
        const val CLIP_FRAC = 0.02f

        /** Перекос, которого достаточно при ограничении: спектр там уже выпрямлен. */
        const val WIND_TILT_CLIPPED = 3f

        /**
         * Глубина подавления у отсечки и у верхнего края полосы. Внизу 0.98,
         * а не 0.95: на сильной струе остатка от прежних −26 дБ было слышно, а
         * речи ниже трёхсот герц всё равно нет — там только основной тон, и его
         * прикрывает нижняя граница ramp'а по наклону.
         */
        const val WIND_DEPTH_LOW = 0.98f
